            ...
            <li v-for="(item, index) in menuItems" :key="index" class="relative group">
                <div class="flex justify-between items-center gap-2">
                    ...
                    <template v-if="item.links && item.links.length">
                        ...
                        <div class="nav fixed top-[var(--header-height)] left-0 z-[1] w-full py-32 border-2 border-t border-[#D4D4D422] bg-white overflow-hidden invisible opacity-0  group-hover:visible group-hover:opacity-100 transition-opacity duration-300 ease-in-out" @mouseleave="hideNav" ref="navMenu">
                                    ...
                                    <div v-for="(subItem, subItemIndex) in item.links" :key="subItemIndex" class="flex flex-col text-black gap-8">
                                        ...
                                        <template v-if="subItem.subLinks && subItem.subLinks.length">
                                            ...
                                            <div class="flex flex-col gap-6">
                                                <NuxtLink v-for="(subLink, subLinkIndex) in subItem.subLinks" :key="subLinkIndex" :to="`/${subItem.path}/${subLink.path}`" @click="hideNav" class="link rounded-full hover:bg-primary/15 cursor-pointer select-none transition-all duration-300 ease-in-out">{{ subLink.title }}dd</NuxtLink>
                                            </div>
                                        </template>
